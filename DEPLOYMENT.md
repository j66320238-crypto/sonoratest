...
```json
{"ok":true,"version":"12.14.1","tmdb_configured":true}
```

### Self-test endpoint (v12.14.1)

`GET /api/selftest` har content upstream ko sach me hit karta hai aur per-API
result deta hai — Cinemeta (movies + series), AniList, Jikan, live-TV catalogue,
aur ek anime episode jisme **Hindi audio track hona zaroori hai**. Deploy ke
baad ek baar chala kar dekh lein:

```bash
curl -s https://<your-app>/api/selftest | jq '.passed, .total'
```

`?quick=1` Hindi-stream check skip karta hai (~4s bachta hai).
UI me bhi hai: **Settings → Automatic API test → Run test now**.
Automatic run **default OFF** hai — koi background traffic nahi jaata jab tak
aap khud On na karein.

`GET /api/health` ab circuit-breaker state bhi deta hai (`breakers[]`), taaki
"anime nahi chal raha" ka asli reason (upstream down vs breaker open) dikh jaye.
Breaker ab half-open hota hai aur ek background prober har 60s me tripped
upstream ko dobara check karke recover hote hi circuit band kar deta hai.
